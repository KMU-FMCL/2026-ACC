...
#include "qcar2_control/vec2.hpp"
#include "qcar2_msgs/msg/ph_quintic_path.hpp"

namespace qcar2_control::ph
{

// Import shared math types from global namespace
using ::Vec2;
using ::dot;
using ::norm;
using ::normalize;
...
private:
  ...
  static double approxSegmentLength(
  ...
  ClosestResult findClosestGlobal(
    const Vec2 & query, int global_seeds_per_segment, int newton_max_iterations,
    double newton_tolerance) const;
  ClosestResult findClosestLocal(
    const Vec2 & query, double s_center, double half_window, int local_seed_count,
