...
// File-local numerical constants for PID controller operations.
static constexpr double kEpsilon = 1e-6;
static constexpr double kMaxDt = 1.0;
static constexpr double kMaxSteerRad = 0.6;

class PidLateralControllerNode : public rclcpp::Node
{
public:
  PidLateralControllerNode()
  : Node("pid_lateral_controller_node")
  {
    ...
    steer_rate_limit_radps_ = this->declare_parameter<double>("steer_rate_limit_radps", 10.0);  // Currently not critical in tuning.
    ...
    // Gain-scheduling parameters: scale gains inversely with speed.
    ...
    // Longitudinal speed control parameters (curvature-based smoothstep).
    ...
    // Output smoothing parameters (EMA: Exponential Moving Average).
    // Lower alpha means stronger smoothing (0.1 smoothest, 1.0 no filtering).
    ...
    sub_guidance_ = this->create_subscription<qcar2_msgs::msg::LateralGuidance>(
      ...
      std::bind(&PidLateralControllerNode::guidanceCallback, this, std::placeholders::_1));

    sub_path_ = this->create_subscription<qcar2_msgs::msg::BezierCurve>(
      "/planning/local_path", rclcpp::QoS(1),
      std::bind(&PidLateralControllerNode::pathCallback, this, std::placeholders::_1));
    ...
    timer_ = this->create_wall_timer(
      ...
      std::bind(&PidLateralControllerNode::update, this));

    // Pre-initialize reusable command message to avoid per-cycle allocations.
    cmd_msg_.motor_names = {"steering_angle", "motor_throttle"};
    cmd_msg_.values.resize(2);

    RCLCPP_INFO(this->get_logger(), "PID lateral controller started");
  }
...
private:
  void guidanceCallback(const qcar2_msgs::msg::LateralGuidance::SharedPtr msg)
  {
    ...
    if (!have_curve_id_ || msg->curve_id != last_curve_id_) {
      ...
      have_curve_id_ = true;
      reset_pid_ = true;
      stopped_ = false;  // Clear stop latch when a new path arrives.
    }
  }
  ...
  void pathCallback(const qcar2_msgs::msg::BezierCurve::SharedPtr msg)
  {
    ...
    const int degree = static_cast<int>(msg->degree);
    if (degree < 1 || msg->control_points.size() != static_cast<size_t>(degree + 1)) {
      have_stop_u_ = false;
      return;
    }
    if (msg->control_points.size() < 2) {
      have_stop_u_ = false;
      return;
    }

    stop_curve_id_ = msg->curve_id;
    stop_degree_ = degree;
    stop_control_points_.clear();
    stop_control_points_.reserve(msg->control_points.size());
    for (const auto & p : msg->control_points) {
      stop_control_points_.push_back({p.x, p.y});
    }
    stop_binom_ = computeBinom(stop_degree_);

    const Vec2 stop_target = stop_control_points_[stop_control_points_.size() - 2];
    ...
    for (int i = 0; i < count; ++i) {
      ...
      const double d = norm(q - stop_target);
      if (d < best_dist) {
        best_dist = d;
        best_u = u;
      }
    ...
    stop_u_ = best_u;
    stop_curve_point_ = evalBezier(stop_u_);
    have_stop_u_ = true;
  }

  // Thin wrapper: delegates to free function in bezier.hpp using stop-point data.
  Vec2 evalBezier(double u) const
  {
    if (stop_degree_ < 1 || stop_control_points_.empty()) {
      return {0.0, 0.0};
    }
    return ::evalBezier(u, stop_control_points_, stop_binom_, stop_degree_);
  }
  ...
  // Between them: use an S-curve for smooth transition.
  ...
  void update()
  {
    ...
    {
      resetPidState();
      publishSmoothedCmd(dt_control, 0.0, 0.0);  // Smoothly ramp down to a stop.
      return;
    }

    // If already latched in stop state, publish immediate stop (no smoothing).
    if (stopped_) {
      publishStopCmd();
      return;
    }
      ...
      latest_guidance_.curve_id == stop_curve_id_)
    {
      ...
      const double dist = norm(q - stop_curve_point_);
      if (latest_guidance_.u_star >= stop_u_ && dist <= stop_distance_m_) {
        stopped_ = true;  // Latch stop state.
        resetPidState();
        publishStopCmd();  // Immediate stop (no smoothing).
    ...
    // Compute longitudinal target first because gain scheduling depends on it.
    ...
    // Gain scheduling: scale down gains at higher speed to reduce oscillation/overshoot.
    ...
    if (gain_schedule_enabled_) {
      ...
      gain_scale = std::clamp(gain_scale, 0.1, 1.1);  // Prevent extreme gain scaling.
    }

    // P term (with gain scheduling)
    const double p_term = gain_scale * kp_ * e;

    // I term with anti-windup and gain scaling.
    double d_term = 0.0;
    if (dt_pid > kEpsilon) {
      integrator_ += e * dt_pid;
      integrator_ = std::clamp(integrator_, i_min_, i_max_);

      // D term with low-pass filter to reduce noise sensitivity
    ...
    // Feed-forward term based on lookahead curvature (fixed distance ahead)
    double ff_term = 0.0;
    if (std::isfinite(latest_guidance_.ff_lookahead_curvature_1pm)) {
      // Ackermann steering: δ = L * κ (wheelbase * curvature)
      ff_term = kff_ * wheelbase_ * latest_guidance_.ff_lookahead_curvature_1pm;
    }

    const double steer_raw = p_term + i_term + d_term + ff_term;

    // Steering rate limiting (keep previous limiter behavior).
    double steer_cmd = std::clamp(steer_raw, -kMaxSteerRad, kMaxSteerRad);
    ...
    // Publish smoothed command (accel/decel limiting + EMA).
    publishSmoothedCmd(dt_control, target_speed, steer_cmd);
  }

  // Helper that applies smoothing before publishing command output.
  void publishSmoothedCmd(double dt, double target_speed, double target_steer)
  {
    // 1) Apply acceleration/deceleration limits to speed command.
    // If dt is invalid, hold previous speed to avoid startup jump.
    double speed_cmd = last_speed_cmd_;
    if (dt > kEpsilon) {
      const double speed_delta = target_speed - last_speed_cmd_;
      if (speed_delta > 0) {
        // Acceleration branch: cap by max_acceleration_.
        const double max_delta = max_acceleration_ * dt;
        if (speed_delta > max_delta) {
          speed_cmd = last_speed_cmd_ + max_delta;
        } else {
          speed_cmd = target_speed;
        }
      } else {
        // Deceleration branch: cap by max_deceleration_.
    ...
    smoothed_speed_ = speed_smooth_alpha_ * speed_cmd + (1.0 - speed_smooth_alpha_) *
      smoothed_speed_;
    smoothed_steer_ = steer_smooth_alpha_ * target_steer + (1.0 - steer_smooth_alpha_) *
      smoothed_steer_;

    // 3) Clamp and publish.
    cmd_msg_.values[0] = std::clamp(smoothed_steer_, -kMaxSteerRad, kMaxSteerRad);
    cmd_msg_.values[1] = std::clamp(smoothed_speed_, -1.0, 1.0);
    pub_cmd_->publish(cmd_msg_);
  }

  // Publish immediate stop and reset smoothing state.
  void publishStopCmd()
  {
    ...
    // Publish immediate zero command.
    cmd_msg_.values[0] = 0.0;  // steering
    cmd_msg_.values[1] = 0.0;  // speed
    pub_cmd_->publish(cmd_msg_);
  }
  ...
  std::vector<Vec2> stop_control_points_;
  std::vector<double> stop_binom_;
  int stop_degree_{0};
  uint32_t stop_curve_id_{0};
  bool have_stop_u_{false};
  double stop_u_{1.0};
  Vec2 stop_curve_point_{0.0, 0.0};
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<PidLateralControllerNode>());
  rclcpp::shutdown();
  return 0;
}
