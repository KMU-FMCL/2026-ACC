...
private:
  ...
  bool findClosestPoint(const Vec2 & p, double & u_star, Vec2 & q_star) const
  {
    ...
    if (have_last_u_ && u_search_window_ > 0.0) {
      u_min = std::clamp(last_u_star_ - u_search_window_, 0.0, 1.0);
      u_max = std::clamp(last_u_star_ + u_search_window_, 0.0, 1.0);
    ...
    u_star = std::clamp(best.u, 0.0, 1.0);
    q_star = best.point;
    return true;
  }
  ...
  void update()
  {
    ...
    const auto [actual_lookahead, max_curv] = computeLookaheadMaxCurvature(
      u_star,
      lookahead_distance);
