...
static constexpr double kNewtonFallbackStep = 0.05;
}  // namespace
...
bool PhPathRuntime::mapS2SegmentU(double s, size_t & seg_idx, double & u) const
{
  ...
  s = std::clamp(s, 0.0, total_length_m_);
  ...
  u = std::clamp((s - seg_s0) / seg_len, 0.0, 1.0);
  return true;
}
...
Vec2 PhPathRuntime::pointAtRatio(double ratio) const
{
  ...
  const double s = std::clamp(ratio, 0.0, 1.0) * total_length_m_;
  return evalAtS(s).p;
}
...
double PhPathRuntime::ratioFromS(double s) const
{
  ...
  return std::clamp(s / total_length_m_, 0.0, 1.0);
}
  ...
  double newton_tolerance) const
{
  ...
  for (int it = 0; it < max_iter; ++it) {
    ...
    const double u_step = (std::isfinite(h) && std::fabs(h) > kEpsilon) ?
      u - g / h :
      u - kNewtonFallbackStep * std::copysign(1.0, g);
    const double u_next = std::clamp(u_step, 0.0, 1.0);
  ...
  double newton_tolerance) const
{
  ...
  if (std::isfinite(best.dist)) {
    ...
    best.s = std::clamp(best.s, 0.0, total_length_m_);
  }
  return best;
}
  ...
  double newton_tolerance) const
{
  ...
  const double s_lo = std::clamp(s_center - half_window, 0.0, total_length_m_);
  const double s_hi = std::clamp(s_center + half_window, 0.0, total_length_m_);
  ...
  ClosestResult & out)
{
  ...
  best =
    findClosestGlobal(query, global_seeds_per_segment, newton_max_iterations, newton_tolerance);
  if (!std::isfinite(best.dist)) {
    return false;
  }
