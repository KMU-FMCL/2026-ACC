...
class Dashboard(Node):
    def __init__(self) -> None:
        ...
        self._cte_ignore_initial_distance_m = float(
            self.get_parameter("cte_ignore_initial_distance_m").value)
        ...
        if self._enable_plot:
            ...
            self._ax_speed.legend(
                loc="upper right", fontsize=8, facecolor=BG_COLOR, edgecolor=GRID_COLOR)
            ...
            self._path_line, = self._ax_map.plot(
                [], [], color=PATH_COLOR, linewidth=2.5, label="path")
            self._traj_line, = self._ax_map.plot(
                [], [], color=TRAJ_COLOR, linewidth=1.5, label="vehicle")
            self._ax_map.set_aspect("equal", adjustable="box")
            self._ax_map.legend(
                loc="upper right", fontsize=9, facecolor=BG_COLOR, edgecolor=GRID_COLOR)
    ...
    def _setup_axis(self, ax):
        """Apply common axis styling for dark theme."""
    ...
    def _path_callback(self, msg: Path) -> None:
        ...
        self._path_xy = []
        for p in msg.poses:
            rx, ry = self._rotate_display(p.pose.position.x, p.pose.position.y)
            self._path_xy.append((rx, ry))
            self._path_x_arr.append(rx)
            self._path_y_arr.append(ry)
        self._limits_set = False
    ...
    def _update_map_limits(self) -> None:
        if not self._path_xy or not self._enable_plot:
            return
        xmin, xmax = min(self._path_x_arr), max(self._path_x_arr)
        ymin, ymax = min(self._path_y_arr), max(self._path_y_arr)
        xc = (xmin + xmax) / 2.0
        yc = (ymin + ymax) / 2.0
    ...
    def _on_timer(self) -> None:
        ...
        if not self._completed and self._distance_travelled > 1.0:
            if abs(self._latest_target_speed) < 0.01:
                ...
                elif (now - self._completion_start).nanoseconds * 1e-9 \
                        >= self._completion_hold_sec:
