...
def _de_casteljau(
    points: Sequence[Tuple[float, float, float]], u: float
...
def _generate_arc_points(
    start: Tuple[float, float, float], apex: Tuple[float, float, float],
    end: Tuple[float, float, float], num_points: int
...
def _load_path_config(
    config_file: str, logger: Any
) -> Tuple[Dict[str, Tuple[float, float, float]], List[Dict[str, str]]]:
    """
    Load waypoints and segments from a YAML config file.

    Args:
        config_file: path to the YAML file, or empty string to use built-in defaults
        logger: ROS logger instance for warning messages

    Returns
    -------
    Tuple of (waypoints dict, segments list)

    """
    if not config_file:
        logger.warning("waypoint_config_file not set, using built-in defaults")
    ...
    try:
        ...
        logger.warning(f"Config file not found: {config_file}, using built-in defaults")
        return (WAYPOINTS, SEGMENTS)
...
WAYPOINTS = {
    ...
    "3": (2.251, -1.100, 0.006),      # Corner 1 apex
    "4": (2.251, -0.229, 0.006),      # Corner 1 end
    ...
    "18": (1.117, 4.395, 0.006),
    "19": (-1.305, 0.795, 0.006),
...
class BezierPathPublisher(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("waypoint_config_file", "")  # Path to YAML config file.
        ...
        config_file = str(self.get_parameter("waypoint_config_file").value)
        waypoints, segments = _load_path_config(config_file, self.get_logger())
        ...
        self._raw_control_points = _generate_path_control_points(
            self._waypoints, self._segments, line_density, arc_density
        )
        self._degree = len(self._raw_control_points) - 1
        self._control_points: List[Tuple[float, float, float]] = []
        self._length_m = 0.0
        self._control_points_msg: List[Point] = []
        self._marker_points_cache: List[Point] = []
        self._path_samples_cache: List[Tuple[float, float, float]] = []
        self._refresh_curve()

        self.get_logger().info(
            f"Generated path with degree={self._degree} "
            f"({len(self._raw_control_points)} control points)")
    ...
    def _approx_length(
        self, points: Sequence[Tuple[float, float, float]], samples: int = 50
    ) -> float:
        if len(points) < 2:
            return 0.0
        if samples < 2:
    ...
    def _regen_raw_points(self, line_density: int, arc_density: int) -> None:
        self._raw_control_points = _generate_path_control_points(
            self._waypoints, self._segments, line_density, arc_density
        )
        self._degree = len(self._raw_control_points) - 1
    ...
    def _refresh_curve(self) -> None:
        ...
        self._control_points_msg = [
            Point(x=p[0], y=p[1], z=p[2]) for p in self._control_points]
        self._marker_points_cache = [
            Point(x=x, y=y, z=z)
            for x, y, z in (
                _de_casteljau(self._control_points, float(i) / float(
                    max(1, self._marker_sample_count - 1)))
                for i in range(max(2, self._marker_sample_count))
            )
        ]
        self._path_samples_cache = [
            _de_casteljau(self._control_points, float(i) / float(
                max(1, self._path_sample_count - 1)))
            for i in range(max(2, self._path_sample_count))
        ]
    ...
    def _on_parameters(self, params):
        ...
        new_config_file = str(self.get_parameter("waypoint_config_file").value)
        ...
        for param in params:
            if param.name == "frame_id":
                ...
                density_changed = True
            elif param.name == "waypoint_config_file":
                new_config_file = str(param.value)
                config_changed = True
        ...
        if config_changed:
            self._waypoints, self._segments = _load_path_config(new_config_file, self.get_logger())
            self._regen_raw_points(new_line_density, new_arc_density)
            self._refresh_curve()
    ...
    def _on_timer(self) -> None:
        ...
        msg.control_points = self._control_points_msg
    ...
    def _build_marker(self) -> Marker:
        ...
        marker.points = self._marker_points_cache
        return marker

    def _build_path(self) -> Path:
        samples = self._path_samples_cache
        count = len(samples)
