...
from qcar2_control.bezier_path_publisher import (
    WAYPOINTS, SEGMENTS, _distance, _lerp, _quadratic_bezier)
...
class PhPathPublisher(Node):
    def __init__(self) -> None:
        ...
        self._stop_break_to_msg_idx: Dict[int, int] = {
            b: i for i, b in enumerate(self._stop_break_indices)}
        ...
        self._phase_internal_stop_prefix_cache: Dict[
            Tuple[int, int],
            Tuple[List[PhQuintic], List[Tuple[float, float, float]]]] = {}
        ...
        if self._sequential_mode:
            self.get_logger().info(
                f"PH path publisher started: sequential mode ON, "
                f"phases={len(self._phase_segment_defs)}, breaks={self._phase_break_indices}"
            )
        else:
            self.get_logger().info(
                f"PH path publisher started: full path ({len(self._all_segments_msg)} segments)")
    ...
    def _split_segments_by_breaks(
        self, segments: Sequence[dict], breaks: List[int]
    ) -> List[List[dict]]:
        if not segments:
            return []
        if not breaks:
    ...
    def _build_preview_points_for(
        self, seg_defs: Sequence[dict]
        ...
        for idx, seg in enumerate(seg_defs):
            ...
            if seg_type == "line":
                ...
                local = [_quadratic_bezier(start, apex, end, float(
                    i) / float(count - 1)) for i in range(count)]
            else:
                continue
    ...
    def _rebuild_cache(self, reset_phase: bool) -> None:
        ...
        self._phase_segment_defs = self._split_segments_by_breaks(
            SEGMENTS, self._phase_break_indices)
        ...
        for phase_idx, phase in enumerate(self._phase_segment_defs):
            self._phase_segments_msg.append(
                self._build_ph_segments_for(phase, self._transformed_waypoints))
            ...
            if phase_idx < len(self._phase_ranges):
                ...
                for stop_break in self._stop_break_indices:
                    if phase_start < stop_break < phase_end:
                        ...
                        prefix_msg = self._build_ph_segments_for(
                            prefix_defs, self._transformed_waypoints)
                        prefix_pts = self._build_preview_points_for(prefix_defs)
                        self._phase_internal_stop_prefix_cache[(
                            phase_idx, stop_break)] = (prefix_msg, prefix_pts)
    ...
    def _publish_stop_event_for_break(self, break_idx: int) -> None:
        ...
        if break_idx not in self._stop_break_indices:
            return
        msg_idx = self._stop_break_to_msg_idx.get(break_idx, -1)
    ...
    def _try_advance_phase(self) -> None:
        ...
        if pending_stop_break is not None:
            ...
            if stop_target is None:
                ...
                if stop_dist <= self._phase_reach_distance_m:
                    ...
                    self._traj_id += 1
                    self.get_logger().info(
                        f"Internal stop hold complete at waypoint {stop_key} "
                        f"(break={pending_stop_break}, traj_id={self._traj_id})"
                    )
                    return
                else:
                    self._internal_stop_reach_start_time = None
        ...
        if dist <= self._phase_reach_distance_m:
            ...
            self.get_logger().info(
                f"PH phase switched: {prev_phase + 1} -> {self._current_phase_idx + 1} "
                f"(traj_id={self._traj_id})"
            )
        else:
            self._phase_reach_start_time = None
    ...
    def _on_parameters(self, params):
        ...
        for param in params:
            if param.name == "frame_id":
                ...
                new_phase_break_indices = self._sanitize_break_indices(
        ...
        if transform_changed or preview_changed or split_changed or stop_changed or mode_changed:
            self._rebuild_cache(reset_phase=(
                transform_changed or split_changed or stop_changed or mode_changed))
            self._traj_id += 1
    ...
    def _on_timer(self) -> None:
        ...
        if self._sequential_mode and self._phase_segments_msg:
            pending_stop_break = self._get_current_internal_stop_break()
            cache_key = (self._current_phase_idx,
                         pending_stop_break) if pending_stop_break is not None else None
